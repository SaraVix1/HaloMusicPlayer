    ...
    F: Fn(MediaControlEvent) + Send + 'static,
{
    ...
    let app_interface = cr.register("org.mpris.MediaPlayer2", {
        ...
        move |b| {
            ...
            // GNOME Shell's media widget needs this to resolve the app icon
            // (via the matching .desktop file's Icon= entry) — it does not
            // fall back to matching on Identity.
            b.property("DesktopEntry")
                .get(move |_, _| Ok(desktop_entry.clone()));
